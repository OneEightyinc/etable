...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'GET') {
    ...
    return res.status(200).json({ account: { id: account.id, name: account.name, email: account.email, storeName: account.storeName, status: account.status, createdAt: account.createdAt } });
  }
